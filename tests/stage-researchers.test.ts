...
describe("stage-owned learnings researchers", () => {
  ...
  test("keeps the retrieval fixture and evaluator rubric separate from forward prompts", async () => {
    ...
    expect(prompts).not.toMatch(/stale\s+writes?/i)
    expect(rubric).toContain("docs/solutions/runtime-errors/token-refresh-race.md")
    expect(rubric).toContain("exact source date: `2026-08-04`")
    expect(rubric).toContain("visible conflict/freshness assessment")
    expect(rubric.indexOf("exact source date: `2026-08-04`")).toBeLessThan(
      rubric.indexOf("visible conflict/freshness assessment")
    )
    ...
    expect(evaluation.slice(preRunStart, postRunStart)).toContain("git status --short")
    expect(evaluation.slice(preRunStart, postRunStart)).toContain("git ls-files docs/solutions")
    expect(evaluation.slice(postRunStart)).toContain("git status --short")
    expect(evaluation.slice(postRunStart)).toContain("git ls-files docs/solutions")
